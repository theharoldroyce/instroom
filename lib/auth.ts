...
const nextAuthConfig = {
    ...
    GoogleProvider({
      ...
      profile(profile: any) {
        ...
        return {
          ...
          name: profile.name,
          email: profile.email,
          image: profile.picture || null, // Explicitly use picture field, fallback to null
        }
      },
    }),
    CredentialsProvider({
  ...
  callbacks: {
      ...
      if (account?.provider === "google" && profile?.email) {
        ...
        try {
          ...
          if (!dbUser) {
            ...
            console.log("Creating user with image:", avatarUrl)
            dbUser = await prisma.user.create({
              data: {
                email: profile.email,
                name: profile.name || profile.email.split("@")[0],
                image: avatarUrl, // Use the extracted avatar URL
                platform_role: "user",
                is_active: true,
              },
            })
            ...
            // Update existing user with latest Google profile info (including avatar)
            console.log("Updating existing user with latest profile info...")
            console.log("Updating user with image:", avatarUrl)
    ...
    jwt({ token, user }: any) {
      if (user) {
        console.log("JWT callback - user signin:", { id: user.id, email: user.email })
        token.id = user.id
    ...
    // Session callback: inject user.id into session
    async session({ session, token }: any) {
      if (session?.user) {
        session.user.id = token.id as string
        console.log("Session callback - user.id set:", { userId: session.user.id })
      }
      return session
    },
  },
