...
function ProfilePicture({ src, socialLink, name, size = 28, onExpired }: { src?: string; socialLink?: string; name?: string; size?: number; onExpired?: () => void }) {
  ...
  const needsProxy = src && (
    ...
    src.includes("tiktokcdn-eu") ||
    src.includes("p16-common") ||
    src.includes("p19-common") ||
    src.includes("muscdn.com") ||
    src.includes("tiktok.com/aweme")
  )
  const proxySrc = src
    ? needsProxy
      ...
      {onAddOption && (
        <div className="border-t border-gray-100 px-2 py-2">
          <div className="flex gap-1">
            <input type="text" value={newOpt} placeholder="Add new…" onChange={e => setNewOpt(e.target.value)} onKeyDown={e => { e.stopPropagation(); if (e.key === "Enter") addNew() }} onMouseDown={e => e.stopPropagation()} className="flex-1 text-sm px-2 py-1 border border-gray-200 rounded outline-none focus:ring-1 ring-indigo-400 min-w-0" />
    ...
    <FloatingPopup onClose={onClose}>
      <div className="w-56 max-h-52 overflow-auto py-1">
        {options.map(opt => {
          ...
          return (
            <button key={opt} onMouseDown={e => e.preventDefault()} onClick={() => toggle(opt)} className={`flex items-center gap-2 w-full text-left px-3 py-1.5 text-sm hover:bg-gray-50 transition ${isOn ? "text-purple-700 font-medium" : "text-gray-700"}`}>
              <span className={`w-4 h-4 rounded border flex items-center justify-center flex-shrink-0 ${isOn ? "bg-purple-600 border-purple-600" : "border-gray-300"}`}>{isOn && <IconCheck size={12} className="text-white" />}</span>{opt}
            </button>
          )
        })}
      ...
      <div className="border-t border-gray-100 px-2 py-2">
        <div className="flex gap-1">
          <input type="text" value={newOpt} placeholder="Add new…" onChange={e => setNewOpt(e.target.value)} onKeyDown={e => { e.stopPropagation(); if (e.key === "Enter") addNew() }} onMouseDown={e => e.stopPropagation()} className="flex-1 text-sm px-2 py-1 border border-gray-200 rounded outline-none focus:ring-1 ring-purple-400 min-w-0" />
...
function exportToCSV(rows: InfluencerRow[], cc: CustomColumn[]): void {
  ...
  const l=rows.map(r=>af.map(f=>{let v="";if(f.key.startsWith("custom."))v=r.custom[f.key.slice(7)]??"";else v=String((r as Record<string,unknown>)[f.key]??"");return escapeCSV(v)}).join(","))
  const csv=[h,...l].join("\n")
...
function ProfileSidebar({ row, customCols, onUpdate, onClose, readOnly=false, niches, locations, onAddNiche, onAddLocation, onToast, brandId }: { row: InfluencerRow|null; customCols: CustomColumn[]; onUpdate: (r: InfluencerRow) => void; onClose: () => void; readOnly?: boolean; niches: string[]; locations: string[]; onAddNiche: (v: string) => void; onAddLocation: (v: string) => void; onToast?: (type: "success"|"error"|"info"|"warning", message: string) => void; brandId?: string }) {
  ...
  const handleSave = async () => {
    ...
    if (row.id.trim() === "") {
      onToast?.("error", "Cannot save: Influencer ID is missing.")
      return
    }
    setIsSaving(true)
    try {
      ...
      const payload = {
        ...
        bio: editedRow.bio || null,
        profile_image_url: editedRow.profile_image_url || null,
        ...
        agreed_rate: editedRow.agreed_rate || null,
        notes: editedRow.notes || null,
      ...
      if (response.ok) {
        ...
        onToast?.("error", error.error || "Failed to save")
      }
    } catch {
      onToast?.("error", "Failed to save. Check your connection.")
    } finally {
      setIsSaving(false)
    }
  }
  ...
  const S = {
    overlay:{position:"fixed" as const,inset:0,background:"rgba(0,0,0,0.3)",zIndex:400},
    panel:{position:"fixed" as const,top:0,right:0,width:520,maxWidth:"100vw",height:"100%",background:"#fff",boxShadow:"-4px 0 24px rgba(0,0,0,0.12)",zIndex:500,display:"flex",flexDirection:"column" as const,fontFamily:"'Inter',system-ui,sans-serif"},
    header:{padding:"16px 20px",borderBottom:"0.5px solid rgba(0,0,0,0.08)"},
    avatar:{width:44,height:44,borderRadius:"50%",background:"#1fae5b",display:"flex",alignItems:"center",justifyContent:"center",fontSize:18,fontWeight:700,color:"#fff",flexShrink:0},
    name:{fontSize:15,fontWeight:600,color:"#1e1e1e"},
    ...
    atag:{fontSize:12,fontWeight:500,padding:"6px 14px",borderRadius:20,cursor:"pointer",border:"1px solid rgba(0,0,0,0.15)",background:"#f7f9f8",color:"#555"},
    ...
    tabBar:{display:"flex",gap:0,padding:"0 20px",borderBottom:"0.5px solid rgba(0,0,0,0.08)"},
    tab:(a:boolean)=>({fontSize:12,fontWeight:500,padding:"10px 14px",cursor:"pointer",color:a?"#1fae5b":"#888",borderBottom:a?"2px solid #1fae5b":"2px solid transparent",whiteSpace:"nowrap" as const}),
    body:{flex:1,overflowY:"auto" as const,padding:"16px 20px"},
    statRow:{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:10,background:"#f7f9f8",borderRadius:10,padding:12,marginBottom:14},
    statBox:{textAlign:"center" as const},
    statLabel:{fontSize:10,color:"#888"},
    statVal:{fontSize:15,fontWeight:600,color:"#1e1e1e",marginTop:2},
    fieldGrid:{display:"grid",gridTemplateColumns:"1fr 1fr"},
    fieldRow:{padding:"8px 0",borderBottom:"0.5px solid rgba(0,0,0,0.05)"},
    fieldLabel:{fontSize:10,color:"#888",marginBottom:2},
    fieldVal:{fontSize:12,color:"#1e1e1e",fontWeight:500},
    ...
    formLabel:{fontSize:10,color:"#888"},
    formInput:{width:"100%",fontSize:12,padding:"8px 10px",borderRadius:8,border:"0.5px solid rgba(0,0,0,0.15)",background:"#fff",color:"#1e1e1e"},
    saveBtn:{background:"#1fae5b",color:"#fff",border:"none",padding:"6px 14px",borderRadius:8,cursor:"pointer",fontSize:11,fontWeight:500},
    sectionTitle:{fontSize:11,fontWeight:600,color:"#555",textTransform:"uppercase" as const,letterSpacing:"0.05em",padding:"10px 0 6px",borderBottom:"0.5px solid rgba(0,0,0,0.06)",marginBottom:10},
  }

  return (
    <>
      <DeclineConfirmationModal isOpen={showDeclineModal} onClose={()=>setShowDeclineModal(false)} onConfirm={(r)=>{if(editedRow)setEditedRow(handleApprovalChange(editedRow,"Declined",r))}} influencerName={editedRow.full_name||editedRow.handle||"this influencer"}/>
      ...
      <div style={S.panel}>
        <div style={S.header}>
          ...
          <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:12}}>
            ...
            <div style={{flex:1}}>
              ...
              <div style={S.handle}>{displayHandle(editedRow.handle, editedRow.platform)}</div>
            </div>
        ...
        <div style={S.tabBar}>
          {["Basic","Order","Post","Stats"].map((tab,idx)=><div key={idx} style={S.tab(profileTab===idx)} onClick={()=>setProfileTab(idx)}>{tab}</div>)}
        </div>
        <div style={S.body}>
          {profileTab===0&&(
            <div style={{display:"flex",flexDirection:"column",gap:14}}>
              <div style={S.statRow}>
                <div style={S.statBox}><div style={S.statLabel}>Followers</div><div style={S.statVal}>{formatFollowers(Number(editedRow.follower_count)||0)}</div></div>
                <div style={S.statBox}><div style={S.statLabel}>Eng Rate</div><div style={S.statVal}>{editedRow.engagement_rate||"0"}%</div></div>
                <div style={S.statBox}><div style={S.statLabel}>Tier</div><div style={S.statVal}>{editedRow.tier||"Bronze"}</div></div>
              ...
              <div style={S.fieldGrid}>
                <div style={S.fieldRow}><div style={S.fieldLabel}>Location</div>{readOnly?<div style={S.fieldVal}>{editedRow.location||"—"}</div>:<select style={{...S.formInput,padding:"4px 8px"}} value={editedRow.location||""} onChange={e=>handleFieldChange("location",e.target.value)}><option value="">—</option>{locations.map(l=><option key={l} value={l}>{l}</option>)}</select>}</div>
                <div style={S.fieldRow}><div style={S.fieldLabel}>Niche</div>{readOnly?<div style={S.fieldVal}>{editedRow.niche||"—"}</div>:<select style={{...S.formInput,padding:"4px 8px"}} value={editedRow.niche||""} onChange={e=>handleFieldChange("niche",e.target.value)}><option value="">—</option>{niches.map(n=><option key={n} value={n}>{n}</option>)}</select>}</div>
                <div style={S.fieldRow}><div style={S.fieldLabel}>Gender</div>{readOnly?<div style={S.fieldVal}>{editedRow.gender||"—"}</div>:<select style={{...S.formInput,padding:"4px 8px"}} value={editedRow.gender||""} onChange={e=>handleFieldChange("gender",e.target.value)}><option value="">—</option><option>Male</option><option>Female</option><option>Non-binary</option><option>Other</option></select>}</div>
                <div style={S.fieldRow}><div style={S.fieldLabel}>Platform</div>{readOnly?<div style={S.fieldVal}>{platformLabel}</div>:<select style={{...S.formInput,padding:"4px 8px"}} value={editedRow.platform} onChange={e=>handleFieldChange("platform",e.target.value)}><option value="instagram">Instagram</option><option value="tiktok">TikTok</option><option value="youtube">YouTube</option><option value="twitter">X (Twitter)</option></select>}</div>
                <div style={S.fieldRow}><div style={S.fieldLabel}>Email</div><div style={S.fieldVal}>{editedRow.contact_info||"—"}</div></div>
                <div style={S.fieldRow}><div style={S.fieldLabel}>Social Link</div><div style={S.fieldVal}>{editedRow.social_link?<a href={normalizeUrl(editedRow.social_link)} target="_blank" rel="noopener noreferrer" style={{color:"#2C8EC4",textDecoration:"none"}}>{editedRow.social_link.replace(/^https?:\/\//,"").slice(0,30)}</a>:"—"}</div></div>
              </div>
              <div style={{marginTop:8}}><div style={S.fieldLabel}>Approval Notes</div>{readOnly?<div style={{fontSize:12,color:"#555",background:"#f7f9f8",borderRadius:8,padding:8,minHeight:40,marginTop:4}}>{editedRow.approval_notes||"No notes"}</div>:<textarea style={{...S.formInput,minHeight:60,resize:"vertical",marginTop:4}} value={editedRow.approval_notes||""} onChange={e=>{handleFieldChange("approval_notes",e.target.value)}} placeholder="Add approval notes..."/>}</div>
              <div style={{marginTop:4}}><div style={S.fieldLabel}>Notes</div>{readOnly?<div style={{fontSize:12,color:"#555",background:"#f7f9f8",borderRadius:8,padding:8,minHeight:60,marginTop:4}}>{editedRow.notes||"No notes"}</div>:<textarea style={{...S.formInput,minHeight:80,resize:"vertical",marginTop:4}} value={editedRow.notes} onChange={e=>handleFieldChange("notes",e.target.value)} placeholder="Add notes..."/>}</div>
              {customCols.length>0&&(
                <div style={{marginTop:4}}>
                  ...
                  <div style={S.fieldGrid}>
                    {customCols.map(col=>{
                      ...
                      return(
                        <div key={col.id} style={S.fieldRow}>
                          <div style={S.fieldLabel}>{col.field_name}</div>
                          {readOnly?<div style={S.fieldVal}>{val||"—"}</div>:col.field_type==="boolean"?<select style={{...S.formInput,padding:"4px 8px"}} value={val} onChange={e=>handleFieldChange(`custom.${col.field_key}`,e.target.value)}><option value="No">No</option><option value="Yes">Yes</option></select>:col.field_type==="dropdown"?<select style={{...S.formInput,padding:"4px 8px"}} value={val} onChange={e=>handleFieldChange(`custom.${col.field_key}`,e.target.value)}><option value="">—</option>{col.field_options?.map(o=><option key={o} value={o}>{o}</option>)}</select>:<input style={{...S.formInput,padding:"4px 8px"}} type={col.field_type==="number"?"number":"text"} value={val} onChange={e=>handleFieldChange(`custom.${col.field_key}`,e.target.value)}/>}
                        </div>
                      )
                    })}
                  </div>
              ...
              {!readOnly&&<div style={{display:"flex",justifyContent:"flex-end",marginTop:8}}><button style={{...S.saveBtn,opacity:isSaving?0.6:1,cursor:isSaving?"not-allowed":"pointer"}} onClick={handleSave} disabled={isSaving}>{isSaving?"Saving...":"Save"}</button></div>}
            </div>
          )}
          {profileTab===1&&(
          ...
          {profileTab===2&&(
            <div style={{display:"flex",flexDirection:"column",gap:10}}>
              <div style={S.formRow}><div style={S.formGroup}><div style={S.formLabel}>Post Link</div><input style={S.formInput} value={postData.postLink} onChange={e=>setPostData(d=>({...d,postLink:e.target.value}))}/></div><div style={S.formGroup}><div style={S.formLabel}>Likes</div><input style={S.formInput} value={postData.likes} onChange={e=>setPostData(d=>({...d,likes:e.target.value}))}/></div></div>
              <div style={S.formRow}><div style={S.formGroup}><div style={S.formLabel}>Sales</div><input style={S.formInput} value={postData.sales} onChange={e=>setPostData(d=>({...d,sales:e.target.value}))}/></div><div style={S.formGroup}><div style={S.formLabel}>Drive Link</div><input style={S.formInput} value={postData.driveLink} onChange={e=>setPostData(d=>({...d,driveLink:e.target.value}))}/></div></div>
              <div style={S.formRow}><div style={S.formGroup}><div style={S.formLabel}>Comments</div><input style={S.formInput} value={postData.comments} onChange={e=>setPostData(d=>({...d,comments:e.target.value}))}/></div><div style={S.formGroup}><div style={S.formLabel}>Amount ($)</div><input style={S.formInput} value={postData.amount} onChange={e=>setPostData(d=>({...d,amount:e.target.value}))}/></div></div>
              <div style={S.formRow}><div style={S.formGroup}><div style={S.formLabel}>Usage Rights</div><select style={S.formInput} value={postData.usageRights} onChange={e=>setPostData(d=>({...d,usageRights:e.target.value}))}><option value="">Select...</option><option>Granted</option><option>Not Granted</option><option>Pending</option></select></div><div style={S.formGroup}><div style={S.formLabel}>Views</div><input style={S.formInput} value={postData.views} onChange={e=>setPostData(d=>({...d,views:e.target.value}))}/></div></div>
              <div style={S.formRow}><div style={S.formGroup}><div style={S.formLabel}>Clicks</div><input style={S.formInput} value={postData.clicks} onChange={e=>setPostData(d=>({...d,clicks:e.target.value}))}/></div><div style={S.formGroup}><div style={S.formLabel}>CVR (auto)</div><input style={{...S.formInput,background:"#f7f9f8",color:"#2C8EC4"}} readOnly value={postCVR}/></div></div>
              <div style={{display:"flex",justifyContent:"flex-end"}}><button style={{...S.saveBtn,opacity:isSaving?0.6:1,cursor:isSaving?"not-allowed":"pointer"}} onClick={handleSave} disabled={isSaving}>{isSaving?"Saving...":"Save"}</button></div>
            </div>
          )}
          {profileTab===3&&(
            <div style={{display:"flex",flexDirection:"column",gap:0}}>
              ...
              <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:8,marginBottom:16}}>
                <div style={{background:"#f7f9f8",borderRadius:8,padding:"10px 12px",textAlign:"center"}}><div style={{fontSize:14,fontWeight:600}}>{editedRow.tier||"Bronze"}</div><div style={{fontSize:10,color:"#888",marginTop:2}}>Tier</div></div>
                <div style={{background:"#f7f9f8",borderRadius:8,padding:"10px 12px",textAlign:"center"}}><div style={{fontSize:14,fontWeight:600,color:editedRow.approval_status==="Approved"?"#1fae5b":editedRow.approval_status==="Declined"?"#e24b4a":"#854f0b"}}>{editedRow.approval_status||"Pending"}</div><div style={{fontSize:10,color:"#888",marginTop:2}}>Approval</div></div>
                <div style={{background:"#f7f9f8",borderRadius:8,padding:"10px 12px",textAlign:"center"}}><div style={{fontSize:14,fontWeight:600}}>{STATUS_LABEL[editedRow.contact_status]||editedRow.contact_status}</div><div style={{fontSize:10,color:"#888",marginTop:2}}>Pipeline</div></div>
              </div>
              {editedRow.transferred_date&&(<div style={{background:"#e6f9ee",borderRadius:8,padding:10,marginTop:12,fontSize:11,color:"#166534"}}><strong>Transferred:</strong> {new Date(editedRow.transferred_date).toLocaleDateString()}</div>)}
            </div>
          )}
        </div>
      </div>
...
/* ═══════════════════════════════════════════════════════════════════════════════
   ...
   ═══════════════════════════════════════════════════════════════════════════════ */
const FIELD_TYPE_INFO: Record<string, { description: string; example: string }> = {
...
export default function TableSheet({
  ...
  const rawCols: AnyColDef[] = [
    ...
    ...customCols.map<CustomColDef>(c => ({
      ...
      group: getEffectiveGroup(c), minWidth: c.field_type==="date"?110:c.field_type==="boolean"?70:100,
  ...
  useEffect(() => {
    ...
    const h = (e: MouseEvent) => {
      if (importExportRef.current && !importExportRef.current.contains(e.target as Node) && importExportBtnRef.current && !importExportBtnRef.current.contains(e.target as Node))
  ...
  const handleAddMultipleRows = (count: number) => {
    ...
    setRows(prev=>{
      let n:InfluencerRow[]
      if(selectedRowIds.size>0){const si=filteredRows.map((r,i)=>selectedRowIds.has(r.id)?i:-1).filter(i=>i!==-1);const li=Math.max(...si);const lid=filteredRows[li].id;const ii=prev.findIndex(r=>r.id===lid)+1;n=[...prev.slice(0,ii),...nr,...prev.slice(ii)]}
      else{n=[...prev,...nr]}
  ...
  const deleteRow = (id: string) => {
    ...
    setConfirmDialog({isOpen:true,title:"Delete Row",message:<span>Delete <strong>{r?.full_name||r?.handle||"this row"}</strong>?</span>,onConfirm:()=>{
  ...
  const deleteSelectedRows = () => {
    ...
    setConfirmDialog({isOpen:true,title:"Delete Selected Rows",message:<span>Delete <strong>{selectedRowIds.size} rows</strong>?</span>,onConfirm:()=>{
  ...
  const onColDragEnd=()=>{
    if(dragIdx!==null&&dragOverIdx!==null&&dragIdx!==dragOverIdx){
      ...
      setColOrder(prev=>{const a=[...(prev??rawCols.map((_,i)=>i))];const[m]=a.splice(dragIdx,1);a.splice(dragOverIdx!,0,m);return a})
    }
  ...
  const isOutreachField = useCallback((colKey: string): boolean => {
    if(colKey.startsWith("custom.")){const fk=colKey.slice(7);const cc=customCols.find(c=>c.field_key===fk);return cc?.assignedGroup==="Outreach Details"}
  ...
  const applyCellValue = useCallback((rowIdx: number, colKey: string, value: string) => {
    ...
    let shouldFetch = false; let fetchRowId = currentRow.id; let fetchHandle = ""; let fetchPlatform = ""
  ...
  const deleteCustomCol = (fk: string) => {
    setConfirmDialog({isOpen:true,title:"Delete Custom Column",message:"Delete this column? All data will be lost.",onConfirm:()=>{
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      {sidebarRow && (
        <ProfileSidebar
          row={sidebarRow} customCols={customCols} onUpdate={handleUpdateRow}
          onClose={()=>setSidebarRowId(null)} readOnly={readOnly}
          niches={nicheOptions} locations={locationOptions}
